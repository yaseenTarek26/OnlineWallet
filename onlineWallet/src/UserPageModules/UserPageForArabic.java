...
public class UserPageForArabic {
    ...
    public static void UserPageForArabic(ResultSet userData) {
        ...
        do {
            ...
            switch (Integer.parseInt(choose)) {
                case 1:
                    printUserData(conn, user, user.getSecurityCode());
                    break;
                case 2:
                    showActivity(conn, user.getUserId());
                    break;
                case 3:
                    try {
                    ...
                    break;
                case 4:
                    deposit(conn, sc, user.getUserId());
                    break;
                case 5:
                    withDraw(conn, sc, user.getUserId());
                    break;
                case 6:
                    user = null;
    ...
    private static void sendMoneyToAnotherAccount(Connection conn, Scanner sc, Users user) throws SQLException, ClassNotFoundException {
        ...
        do {
            do {
                depositNumber = sc.next();
                if (!userServiceAndValidationInputsArabicSub.isNumber(depositNumber))
                    System.out.println("مدخل غير صالح");
            } while (!userServiceAndValidationInputsArabicSub.isNumber(depositNumber));
    ...
    private static void Settings(Connection conn, Scanner sc, Users user) {
        ...
        switch (userOption) {
            case 1:
                userServiceAndValidationInputsArabicSub.updateUserName(conn, sc, user.getUserId());
                break;
            case 2:
                userServiceAndValidationInputsArabicSub.updateUserPassword(conn, sc, user.getUserId());
                break;
            case 3:
                userServiceAndValidationInputsArabicSub.updateUserEmail(conn, sc, user.getUserId());
                break;
            case 4:
                userServiceAndValidationInputsArabicSub.updateUserPhone(conn, sc, user.getUserId());
                break;
            case 5:
                return;
        }
